...
/*
Name              Poly         Init-value    Reverse   XOR-out
CRC-32/ISO-HDLC   0x04C11DB7   0xFFFFFFFF    true      0xFFFFFFFF
...
#define CRC32_REVERSE    true
#define CRC16_POLYNOMIAL 0x8005   // CRC16_MODBUS
#define CRC16_INITIAL    0xFFFF
#define CRC16_XOROUT     0x0000
#define CRC16_REVERSE    true

#define INIT_CRC_TABLE 0
#define MAIN_TEST      0
...
#if INIT_CRC_TABLE
static uint32_t reverse_bits(uint32_t num, int num_bits)
{
    uint32_t result = 0;
    for (int i = 0; i < num_bits; i++) {
        result = (result << 1) | (num & 0x1);
        num >>= 1;
    }
    return result;
...
static void crc16_init_table()
{
    ...
    uint16_t crc;
    if (CRC16_REVERSE) {
        poly = reverse_bits(CRC16_POLYNOMIAL, 16);
...
int main()
{
    const char data[] = "jajg;jeirgkndkj.njshglehglbbns;afgb.";
    uint32_t   crc;
    ...
    crc = calc_crc16(data, sizeof(data) - 1);
    printf("crc16=%04x\n", crc);

    crc = calc_crc32_pre();
    crc = calc_crc32_step(data, 10, crc);
    crc = calc_crc32_step(&data[10], sizeof(data) - 10 - 1, crc);
    crc = calc_crc32_finish(crc);
    printf("crc32=%08x\n", crc);

    crc = calc_crc32(data, sizeof(data) - 1);
    printf("crc32=%08x\n", crc);
    ...
    printf("};\n");
    printf("static uint32_t crc16_table[256]={");
    for (int i = 0; i < 256; i++) {
